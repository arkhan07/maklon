...
<div class="p-8 max-w-5xl mx-auto space-y-12">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <a href="#paket" class="bg-primary text-white px-6 py-3 rounded-lg font-bold shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5 whitespace-nowrap">
            Mulai Pendaftaran
        </a>
    </div>
    ...
    <div id="paket">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-8 hover:shadow-lg transition-shadow">
                ...
                <form method="POST" action="{{ route('verification.buy_package') }}">
                    @csrf
                    <input type="hidden" name="package_type" value="pt_perorangan">
                    <button type="submit" class="w-full bg-slate-100 dark:bg-slate-800 text-primary dark:text-white py-3 rounded-lg font-bold mb-8 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                        Pilih Basic
                    </button>
                </form>
                <ul class="space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border-2 border-primary p-8 shadow-xl relative">
                ...
                <form method="POST" action="{{ route('verification.buy_package') }}">
                    @csrf
                    <input type="hidden" name="package_type" value="pt_perseroan">
                    <button type="submit" class="w-full bg-primary text-white py-3 rounded-lg font-bold mb-8 hover:bg-slate-800 transition-colors shadow-md">
                        Pilih Premium
                    </button>
                </form>
                <ul class="space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-primary/5 dark:bg-slate-900 border border-primary/10 dark:border-slate-800 rounded-xl p-6">
            ...
            <div class="space-y-3">
                <a href="https://wa.me/6281234567890?text=Halo+Maklon.id%2C+saya+ingin+konsultasi+mengenai+layanan+pendirian+PT+dan+legalitas."
                   target="_blank"
                   class="w-full flex items-center justify-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-2.5 rounded-lg text-sm font-bold text-primary dark:text-white hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                ...
                <a href="#paket" class="w-full flex items-center justify-center py-2.5 rounded-lg text-sm font-bold text-primary dark:text-blue-400 hover:underline">
                    Lihat Paket Lengkap
                </a>
            </div>
        </div>
    ...
    <div class="bg-primary rounded-2xl p-8 md:p-12 text-center text-white overflow-hidden relative mb-4">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center relative z-10">
            <a href="#paket" class="bg-white text-primary px-8 py-3 rounded-lg font-bold hover:bg-blue-50 transition-colors">
                Mulai Sekarang
            </a>
            <a href="https://wa.me/6281234567890?text=Halo+Maklon.id%2C+saya+ingin+tahu+lebih+lanjut+tentang+layanan+pendirian+PT."
               target="_blank"
               class="bg-transparent border border-white/30 text-white px-8 py-3 rounded-lg font-bold hover:bg-white/10 transition-colors">
